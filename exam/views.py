...
from . models import exam_course
...
# def exam_avg_marks(request):
#     exam_marks = Result.objects.values('exam_id').annotate(avg_marks=Avg('marks'))
#     exam_ids = [exam['exam_id'] for exam in exam_marks]
#     avg_marks = [exam['avg_marks'] for exam in exam_marks]
#     return render(request, 'exam_avg_marks.html', {'exam_ids': exam_ids, 'avg_marks': avg_marks})


# @login_required(login_url='adminlogin')
# def teacher-view-marks(request):
#     students= SMODEL.Student.objects.all()
#     return render(request,'exam/teacher-view-marks.html',{'students':students})




def admin_add_companies(request):
    if request.method == 'POST':
        companyForm = CompanyForm(request.POST)
        if companyForm.is_valid():
            companyForm.save()
            return HttpResponseRedirect(reverse('admin_view_companies'))
        else:
            print("form is invalid")
    else:
        companyForm = CompanyForm()
    return render(request, 'exam/admin_add_companies.html', {'companyForm': companyForm})

def admin_view_companies(request):
    companies = Company.objects.all()
    return render(request, 'exam/admin_view_companies.html', {'companies': companies})
...
def student_companies(request):
    companies = Company.objects.all()
    return render(request,'student/student_companies.html', {'companies': companies})
...
def admin_dashboard_view(request):
    # exam_course = exam_course.objects.all (SELECT course_name,total_marks FROM exam_course)
    dict={
...
def admin_companies(request):
    return render(request,'exam/admin_companies.html')
...
def faculty_view_students(request):
    students = SMODEL.Student.objects.all()
    return render(request, 'teachers.html', {'students': students})
